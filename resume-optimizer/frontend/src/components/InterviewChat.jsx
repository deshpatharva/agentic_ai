...
export default function InterviewChat({ onComplete }) {
  ...
  async function finishInterview(history) {
    setFinishError(false);
    try {
      const { data: sections } = await client.post('/profile/ai-interview/finish', { history });
  ...
  return (
    <div className="flex flex-col bg-card border border-line shadow-card rounded-card overflow-hidden">
      <div ref={scrollRef} className="h-96 overflow-y-auto px-4 py-4 flex flex-col">
        ...
        {loading && (
          <div className="flex items-start gap-2 mb-3">
            ...
            <div className="bg-card border border-line shadow-card rounded-card rounded-bl-sm px-4 py-2.5">
              <div className="flex items-center gap-1.5">
                {[0, 150, 300].map((delay) => (
                  <span key={delay} className="w-1.5 h-1.5 rounded-full bg-ink-faint animate-pulse" style={{ animationDelay: `${delay}ms` }} />
                ))}
              </div>
            </div>
          </div>
        ...
        {done && !finishError && (
          <p className="text-center text-xs text-ink-faint mt-2">Interview complete — generating your profile…</p>
        )}
        {done && finishError && (
          <div className="text-center mt-2">
            <p className="text-xs text-err mb-2">Couldn't generate your profile from the interview.</p>
            <button
              type="button"
      ...
      {!done && (
        <div className="border-t border-line px-3 py-2.5 flex items-end gap-2">
          <textarea
            ...
            rows={1}
            disabled={loading}
            className="flex-1 resize-none rounded-lg border border-line bg-card px-3 py-2 text-sm text-ink placeholder:text-ink-faint focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary/30 transition-colors"
